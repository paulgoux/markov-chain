...
vectorText words;
vectorText words2 = new vectorText(sentence);

void setup(){
  size(1000,660);
   //text = loadStrings("lionwitchwardrobe.txt");
   text = loadStrings("bible.txt");
...
void draw(){
  ...
  //words.readText();
  //words.plotText();
  words.displaySimilar(s,1,0);
  words.createSentence(s2,2);
  ////words.findSimilar(s,4,0);
  ////words.findLeastSimilar(s,0.08);
  //words.getFrequency(s);
  //words.getVariance(s);
  //words.getEncodings();
  text(words.maxVariance,600,100);
  text(words.maxFrequency,600,110);
  //text(words.Sentences.size(),100,110);
  //text(words.words.size(),100,120);
};
...
class vectorText{
  ...
  boolean init = true,textcompleted,inputs_complete,separatewords,separatesentences,trimchars,createwordobj,textjoined,getvariance,foundS;
...
ArrayList<Word> findSimilar(String a,int b,float c){
  ...
  if(words1dsingle.contains(a)){
    ...
    if(!foundS){
      ...
      for(int i=0;i<w1.ypos.size();i++){
        ...
        for(int j=w1.xpos.get(i)+1;j<w1.xpos.get(i)+2;j++){
          
          if(j>=0&&j<s.size()&&j<=c+6){
          int k2 = words1dsingle.indexOf(s.get(j));
          
          Word w2 = Words1dsingle.get(k2);
          if(w1.a!=w2.a&&w1.a!=w2.a + " "&&w1.a+" "!=w2.a&&w2.a != " "&&!unsortedSearch.contains(w2)){
  ...
  text(unsortedSearch.size(),10,40);
  return temp;
};
   ...
   if(words1dsingle.contains(Start)){
     ...
     if(newSentence.size()==0){
       ...
       for(int i=0;i<100;i++){
         ...
         //ArrayList<Word> unsortedSearch = null;
         
         ArrayList<Word> wordPool = findSimilar(start.a,1,float(newSentence.size()));
           
           if(wordPool.size()>0){
           Word nextWord = wordPool.get(floor(random(wordPool.size())));
           newSentence.add(nextWord);
           start = nextWord;
           //String end = nextWord.a.substring(nextWord.a.length()-1, nextWord.a.length());
